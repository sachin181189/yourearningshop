 <?php echo $__env->make('header', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>;
 <div class="wishlist-area d-md-block mb-100" style="margin-bottom:50px">
  <div class="container-wrapper pl-15 pr-15">
    <section class="order-complete-area pattern-bg pt-50 pb-50">
      <div class="container">
        <div class="row justify-content-center" style="margin:20px 0px">
          <!-- <div class="col-xl-1 col-lg-1"></div> -->
          <div class="col-xl-12 col-lg-12" style="background:#f6f6f6;padding:50px 0px">
            <div class="order-complete-bg">
              <div class="order-complete-content text-center p-4">
                <h2>Your Order Is Confirmed !</h2>
                <p><?php echo e(Session::get('order_id')); ?></p>
                <h4 class="ml-5 mr-5">Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod
                  tempor incididunt ut labore et dolore magna aliqua.
                </h4>
                <h4>Get answers to all <a href="#" style="color: hotpink"><sup>Questions</sup></a> you might have</h4>
              </div>
            </div>
          </div>
          <!-- <div class="col-xl-1 col-lg-1"></div> -->
        </div>
      </div>
    </section>

  </div>
</div>
 <?php echo $__env->make('footer', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>;
<?php /**PATH C:\xampp\htdocs\yourearningshop\resources\views/payment_success.blade.php ENDPATH**/ ?>